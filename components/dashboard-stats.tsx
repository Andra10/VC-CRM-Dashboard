      ...
      {stats.map((stat, index) => (
        ...
        >
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <stat.icon className="h-4 w-4 text-muted-foreground hover:scale-110 transition-transform duration-200" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-foreground hover:text-primary transition-colors duration-200">
              {stat.value}
            </div>
            <div className="flex items-center space-x-2 text-xs text-muted-foreground">
              <div className="flex items-center space-x-1">
                {stat.trend === "up" ? (
                  <TrendingUp className="h-3 w-3 text-success animate-pulse" />
                ) : (
                  <TrendingDown className="h-3 w-3 text-destructive animate-pulse" />
                )}
